...
public class HotelApi : IApi
{
    ...
    {
        return Results.Extensions.Xml(await repository.GetHotelsAsync());
    }

    [Authorize]
    private async Task<IResult> GetById(int id, IHotelRepository repository)
    {
        return await repository.GetHotelAsync(id) is Hotel hotel
            ? Results.Ok(hotel)
            : Results.NotFound();
    }
    ...
    {
        return await repository.GetHotelsAsync(query) is IEnumerable<Hotel> hotels
            ? Results.Ok(hotels)
